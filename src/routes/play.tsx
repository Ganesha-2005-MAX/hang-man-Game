...
const TIERS: { 
  ...
  { key: "easy", title: "Easy", emoji: "🙂", lives: "6 Lives", words: "Short Words", level: "Beginner Friendly", theme: "success" },
  { key: "medium", title: "Medium", emoji: "🤔", lives: "6 Lives", words: "Longer Words", level: "Good Challenge", theme: "warning", recommended: true },
  { key: "hard", title: "Hard", emoji: "😈", lives: "4 Lives", words: "Challenging Words", level: "For Experts", theme: "destructive" },
];
  ...
  return (
    <div className="min-h-screen bg-transparent p-6 flex flex-col items-center">
      <div className="w-full max-w-5xl">
        <button onClick={() => navigate({ to: "/dashboard" })} className="flex items-center gap-2 text-foreground font-semibold hover:text-primary transition-colors">
          <ArrowLeft className="w-4 h-4" /> Back to Dashboard
        </button>
      </div>

      <div className="max-w-5xl w-full text-center mt-8">
        <h1 className="text-2xl font-bold text-foreground/80 mb-1">Choose your challenge level and start guessing!</h1>
          ...
          {TIERS.map(t => {
            ...
            const themeClasses = {
              success: {
                ...
                title: "text-success",
                iconCircle: "bg-gradient-to-b from-success/40 to-success shadow-[0_0_30px_oklch(0.65_0.18_145/0.3)]",
              ...
              warning: {
                ...
                title: "text-warning",
                iconCircle: "bg-gradient-to-b from-warning/40 to-warning shadow-[0_0_30px_oklch(0.78_0.16_75/0.3)]",
              ...
              destructive: {
                ...
                iconCircle: "bg-gradient-to-b from-destructive/40 to-destructive shadow-[0_0_30px_oklch(0.6_0.22_25/0.3)]",
            ...
            return (
              ...
              >
                ...
                <div className={`w-28 h-28 rounded-full grid place-items-center mb-6 transition-transform duration-500 group-hover:scale-110 ${themeClasses.iconCircle}`}>
                  <span className="text-6xl drop-shadow-lg">{t.emoji}</span>
                </div>

                <h3 className={`text-4xl font-black mb-2 tracking-tight ${themeClasses.title}`}>{t.title}</h3>
                ...
                <div className="space-y-3 w-full">
                  <FeatureRow icon={<Heart className="w-4 h-4" />} text={t.lives} className={themeClasses.row} />
                  <FeatureRow icon={<BookOpen className="w-4 h-4" />} text={t.words} className={themeClasses.row} />
                  <FeatureRow icon={<BarChart3 className="w-4 h-4" />} text={t.level} className={themeClasses.row} />
                </div>
              </button>
            );
          })}
        ...
        <div className="mt-16 flex flex-col items-center gap-6 pb-12">
          ...
          <button onClick={() => navigate({ to: "/dashboard" })} className="text-xl font-bold text-foreground/60 hover:text-foreground transition-colors">
            Cancel
          </button>
        </div>
      </div>
...
function FeatureRow({ icon, text, className }: { icon: React.ReactNode; text: string; className: string }) {
  return (
    <div className={`flex items-center gap-4 px-5 py-3.5 rounded-2xl font-bold text-base shadow-sm transition-transform hover:scale-[1.02] ${className}`}>
